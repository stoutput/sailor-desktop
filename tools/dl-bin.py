...
for owner_repo, files in urls.items():
    ...
    versions[ftarget.upper().replace("-", "_")] = version
    ...
    for platform, fname in files.items():
        ...
        # Download binary
        response = requests.get(
            f"https://github.com/{owner_repo}/releases/download/{version}/{fname}",
        ...
        os.chmod(
            fpath, os.stat(fpath).st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH
        )

# Update .versions file
env_values = {
    f"REACT_APP_{name}_VERSION": version for name, version in versions.items()
}
f1 = open(".env", "r")
f2 = open(".env.bak", "w")
for line in f1:
    ...
    if env_key in env_values:
        f2.write(f"{env_key}={env_values[env_key]}\n")
    else:
        f2.write(line + "\n")
f1.close()
f2.close()
